...
You will need to load two types of data:  the map data (analogous to shape files in GIS tools) and the GCAM data.  The map data can be loaded from the package using `import_mapdata()`.  The first line of this example creates the file path to the map data, and the second loads it as a Simple Feature (sf) object.  This sf object is what you will pass to the plotting functions.  Most of the data you will need are provided as datasets in this package, but you can easily provide your own from a Shapefile or geojson file:
```{r load.map}
