...
if(!requireNamespace('sf', quietly=TRUE)) {
    stop('The sf package is required to build this vignette') # Not sure why...
}
```
...
library('sf') # TODO: Figure out how to rebuild examples without loading this library
```

## Loading GCAM data

You will need to load two types of data:  the map data (analogous to shape files in GIS tools) and the GCAM data.  The map data can be loaded from the package using `import_mapdata()`.  The first line creates the file path to the map data, and the second loads it as a Simple Feature (sf) object.  This sf object is what you will pass to the plotting functions.  Most of the data you will need are provided as datasets in this package, but you can easily provide your own from a Shapefile or geojson file:
```{r load.map}
...
After loading the query you want to display, you must add the region identifiers used in the map data to the data frame using the `add_region_ID()` function.  Then you can join the data and the map using `merge()`.
...
plot_GCAM(mapfile, col = 'region_id', proj = eck3, colorfcn = qualPalette)
```
